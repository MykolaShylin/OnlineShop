...
{
    public class ProxyProductsDbStorage : IProductsStorage
    {
        private readonly DataBaseContext dataBaseContext;
        private readonly ProductsDbStorage _productDbStorage;
        private Product _product;
        private List<Product> _products;
        private List<Product> _categoryProducts;
        private List<Product> _brandProducts;
        public ProxyProductsDbStorage(DataBaseContext dataBaseContext)
        {
            this.dataBaseContext = dataBaseContext;
            _productDbStorage = new ProductsDbStorage(dataBaseContext);
        }
        ...
        {
            var updateProduct = _product ?? await TryGetByIdAsync(product.Id);
        ...
        {
            return _product ??= await _productDbStorage.TryGetByIdAsync(id);
        }
        public async Task<List<Product>> TryGetByCategoryAsync(ProductCategories category)
        {
            return category == ProductCategories.None ? await GetAllAsync() : _categoryProducts ??= await _productDbStorage.TryGetByCategoryAsync(category);
        }

        public async Task<List<Product>> TryGetByBrandAsync(ProductBrands brand)
        {
            return _brandProducts ??= await _productDbStorage.TryGetByBrandAsync(brand);
        }
        public async Task<Product> TryGetByNameAsync(string name)
        {
            return _product ??= await _productDbStorage.TryGetByNameAsync(name);
        }
        public async Task<List<Product>> GetAllAsync()
        {
            return _products??= await _productDbStorage.GetAllAsync();
